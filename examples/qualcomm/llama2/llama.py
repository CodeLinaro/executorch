...
def calibrate(example_inputs, module: torch.fx.GraphModule, tokenizer_model_path):
...
class SingleLlama:
    def __init__(self, llama_model, args) -> None:
        super().__init__()
    ...
    def split_llama(self):
        ...
        with torch.no_grad():
            self.split_modules.append(self.llama_model)
            self.split_inputs.append((tokens, pos_ids, atten_mask, *k_caches, *v_caches))
...
if __name__ == "__main__":
    ...
    composite_llama = SingleLlama(llama_instance.eval(), args)
    kv_input_numel = (
    ...
    if args.pre_gen_pte is None:
        ...
        del llama_instance
        print("Lowering model....")


        print("*************** NOTE!!!!")
        print("We don't have runner for a llama2 without split.")
        print("If you want to run the model on a device, please use composite_llama.py")
        print("Exiting...")
        sys.exit(0)
