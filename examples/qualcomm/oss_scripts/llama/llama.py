...
def smart_mask_updater(ar_len, atten_mask, pos, k_caches, v_caches, new_k_caches, new_v_caches):
    if pos >= ar_len:
...
def _kv_calibrate(
    ...
    with torch.no_grad():
        while token_list[-1] != tokenizer.eos_id and pos < max_seq_len:
            tmp_token_list = torch.tensor(token_list[pos-ar_len: pos], dtype=dtype).reshape(1, -1)
            ...
            if pos < ar_len:
                ...
                tmp_atten_mask = torch.cat([torch.ones(1, ar_len, max_seq_len-pos)*-255.0, atten_mask[:,:,-pos:]], dim=-1)
...
class SingleLlama:
    ...
    def _tag_ios(self, gm: torch.fx.GraphModule, fixed_point_type):
        ...
        atten_mask_shape = {
            (self.llama_meta["get_max_batch_size"], self.llama_meta["get_ar_len"], self.llama_meta["get_max_seq_len"]),
        }
...
def compile(args, pte_filename, tokenizer):
    ...
    with open(args.params) as f:
        ...
        prefill_config.use_kv_cache = False if args.max_seq_len == args.prefill_ar_len else True
...
def _build_parser():
    ...
    parser.add_argument(
        "--prefill_ar_len",
        help="The auto-regression (AR) length determines the number of tokens to consume and the number of logits to produce. Use this option to process the prompt and generate the key-value (kv) cache, which serves as a prompt generator for hybrid mode.",
        default=32,
        type=int,
    )
