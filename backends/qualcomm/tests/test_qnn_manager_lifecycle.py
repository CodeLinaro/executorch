...
class TestQnnManagerLifecycle(unittest.TestCase):
    ...
    def test_multi_soc_lookup_rejects_multiple_backend_options_per_target(self):
        specs = generate_qnn_executorch_compiler_spec(
            soc_model=[QcomChipset.SM8650, QcomChipset.SM8750],
            backend_options=[
                generate_htp_compiler_spec(use_fp16=False),
        ...
        option = flatbuffer_to_option(specs[0].value)
        option.target_options.targets[0].backend_options.append(
            generate_htp_compiler_spec(use_fp16=True)
        )
        specs = [CompileSpec(specs[0].key, option_to_flatbuffer(option))]

        with self.assertRaisesRegex(ValueError, "exactly one backend option"):
            lifecycle.get_current_qnn_manager(specs, QcomChipset.SM8650)
