...
class PyQnnManager {
 public:
  ...
  py::bytes GetDlcBinary(uintptr_t dlc_handle) {
    ...
    if (qnn_manager_->GetDlcBinary(reinterpret_cast<void*>(dlc_handle), binary) != Error::Ok) {
      throw std::runtime_error("Failed to get QNN DLC binary");
    }
    return py::bytes(reinterpret_cast<const char*>(binary.data()), binary.size());
  }

  py::int_ GetDlcRecordCount(uintptr_t dlc_handle, uint32_t record_type) {
    uint32_t count = 0;
    if (qnn_manager_->GetDlcRecordCount(
            reinterpret_cast<void*>(dlc_handle),
            static_cast<QnnSystemDlc_RecordType_t>(record_type),
            count) != Error::Ok) {
      throw std::runtime_error("Failed to inspect QNN DLC");
    }
    return count;
  }
