...
def processed_binary_to_flatbuffer(processed_binary: bytes) -> bytes:
    def md5sum(binary: bytes):
        import hashlib
    ...
    return _convert_to_flatbuffer(
        ProcessedBinaryInfo(
            hash=md5sum(processed_binary),
            data=processed_binary,
        ),
        "qc_processed_binary",
    )
...
def flatbuffer_to_processed_binary_info(flatbuffers: bytes) -> ProcessedBinaryInfo:
    return _convert_to_object(flatbuffers, QnnExecuTorchOptions, "qc_processed_binary")
